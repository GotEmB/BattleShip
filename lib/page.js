...
var async, bindMouseDown, canvas, easeInOut, gotoGameOverMenu, hideFriendPlacingMenu, pleaseWait, roundTo1, setupCanvas, setupEvents, setupGame, setupPage, showDialog, showFriendPlacingMenu, socket, viewController, viewport, wdp, _ref,
...
async = function(a, b) {
  var func, timeOut;
  func = typeof a === "function" ? a : typeof b === "function" ? b : null;
  timeOut = typeof a === "number" ? a : typeof b === "number" ? b : null;
  console.log({
    func: func != null ? func : null,
    timeOut: timeOut != null ? timeOut : null
  });
  if (func == null) return;
  return setTimeout(func, timeOut != null ? timeOut : 0);
};
...
viewController = (function() {
  ...
  viewController.viewStack = ["mainMenu"];

  viewController.push = function(viewId, animate) {
    if (animate != null ? animate : true) {
      $("#menuView").addClass("enableTransitions");
    } else {
      $("#menuView").removeClass("enableTransitions");
    }
    $("#" + (_(viewController.viewStack).last())).addClass("moveLeft");
    $("#" + viewId).removeClass("moveRight");
    return viewController.viewStack.push(viewId);
  };

  viewController.pop = function(animate) {
    if (animate != null ? animate : true) {
      $("#menuView").addClass("enableTransitions");
    } else {
      $("#menuView").removeClass("enableTransitions");
    }
    $("#" + (_(viewController.viewStack).last())).addClass("moveRight");
    viewController.viewStack.pop();
    return $("#" + (_(viewController.viewStack).last())).removeClass("moveLeft");
  };

  viewController.popTo = function(viewId, animate) {
    var _results;
    $("#menuView").removeClass("enableTransitions");
    if (_(viewController.viewStack).last() !== viewId) {
      viewController.pop(animate != null ? animate : true);
    }
    _results = [];
    while (_(viewController.viewStack).last() !== viewId) {
      _results.push(viewController.pop(false));
    }
    return _results;
  };

  viewController.hideViewContainer = function(animate) {
    if (animate != null ? animate : true) {
  ...
  viewController.showViewContainer = function(animate) {
    if (animate != null ? animate : true) {
      $("#menuViewContainer").addClass("enableTransitions");
    } else {
      $("#menuViewContainer").removeClass("enableTransitions");
...
window.onpopstate = function(e) {
  ...
  return viewController.showViewContainer();
};
...
setupEvents = function() {
  ...
  return bindMouseDown("#dialogOkay_btn", function(e) {
    return viewController.hideViewContainer();
  });
};

setupGame = function() {
  async(550, function() {
    return viewController.popTo("mainMenu");
  });
...
setupCanvas = function(data) {
  ...
  Game = (function() {
    ...
    Game.Board = (function() {
      ...
      Board.prototype.position = function() {
        var _this = this;
        return (function(mtx) {
          return new Point([mtx._tx, mtx._ty]);
        })(this.placed._matrix);
      };
  ...
  (function() {
    var placeTool, prevPos, roundCoordinates, selectedShip, shipMover, validator;
    ...
    roundCoordinates = function(point) {
      return new Point({
    ...
    placeTool.onMouseDown = function(e) {
      ...
      if (e.point.isInside(_this.rotate_p.bounds)) {
        ...
        for (str in _ref2) {
          ...
          if (ship.visible && e.point.subtract(Game.mine.board.position()).isInside(ship.gridBounds())) {
            selectedShip.deselect();
            selectedShip = ship;
            selectedShip.select();
            shipMover.currentPos = selectedShip.position;
    ...
    placeTool.onMouseDrag = function(e) {
      var delta;
    ...
    placeTool.onMouseUp = function(e) {
  ...
  (function() {
    ...
    activeTool.onMouseDown = function(e) {
      ...
      if (_(Game.yours.board.back.children).any(function(x) {
        return (x.symbol != null) && x.symbol === _this.redBack_s && e.point.subtract(Game.yours.board.position()).isInside(x.bounds);
      })) {
        cursorOn = false;
        view.onFrame = null;
        return _this.cursor.remove();
      } else if (cursorOn && e.point.subtract(Game.yours.board.position()).isInside(_this.cursor.bounds)) {
        cursorOn = false;
        view.onFrame = null;
        _this.cursor.remove();
        inactiveTool.activate();
        ...
        return socket.emit("kaboom", coordinates, function(result) {
          ...
          switch (result.result) {
            case "hit":
              ...
              _this.shot_s.place(gridPosition);
              break;
            case "sunk":
          ...
          async(1000, result.result === "miss" ? yourTurn : result.result !== "gameOver" ? (function() {
        ...
        flags = _(Game.yours.board.front.children).filter(function(x) {
          return (x.symbol != null) && x.symbol === _this.flag_s && gridPosition.isInside(x.bounds);
        });
        if (flags.length === 0) {
          Game.yours.board.front.activate();
    ...
    return socket.on("shotAt", function(data) {
      ...
      switch (data.result.result) {
        case "hit":
          ...
          _this.redShot_s.place(gridPosition);
          break;
        case "sunk":
      ...
      async(1000, data.result.result === "miss" ? myTurn : void 0);
      return view.draw();
    });
  })();
  return view.draw();
